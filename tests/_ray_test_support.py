...
def setup_worker() -> None:
    ...
    Applies all container-compatibility patches on each Ray worker process, and
    installs the fake LanceDB Cloud/Enterprise backend so ``db://`` tests
    execute through real Ray workers rather than only on the driver.
    """
